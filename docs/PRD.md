...
Model ini spesifikasi target, bukan daftar seluruh fitur yang sudah diimplementasikan. supabase/migrations/ adalah source of truth definisi schema; database_schema.sql lama adalah snapshot arsip historis. Enam tabel foundation dan empat tabel mock memiliki migration sebelumnya. Migration persistence P1.4 (`20260922090000_create_episode_persistence.sql`) telah terpasang pada Supabase PostgreSQL lokal sebelum verifikasi 25 September 2026; agent tidak menerapkannya pada sesi verifikasi tersebut, dan bukti lokal tidak menyatakan migration sudah diterapkan pada environment lain.
...
- P1.3 lifecycle episode (revisi 22 September 2026, rule `episode-lifecycle-v2`) diverifikasi pada 25 September 2026: [review penutupan P1.3](P1_3_REVIEW.md), [kontrak lifecycle](EPISODE_LIFECYCLE.md), dan [bukti pengujian](evidence/P1_3/p13-verification-evidence.md). Versi request, primary/target, rekonsiliasi identitas, riwayat berdasarkan closedAt dengan tie-break leksikal, audit inbound/staf, split dan suppression terbukti melalui 26 unit test domain murni. Fungsi domain murni tanpa I/O atau clock; dispatchAuthorized=false.
- P1.4 persistence lokal diverifikasi pada 25 September 2026: [review P1.4](P1_4_REVIEW.md) dan [bukti integrasi](evidence/P1_4/p14-verification-evidence.md). Migration `20260922090000_create_episode_persistence.sql` telah terpasang pada Supabase lokal sebelum verifikasi. 11 skenario integrasi (12 checks PASS termasuk pengujian induk) membuktikan transaksi PostgreSQL episode/audit atomik, ingress/job atomik, claim atomik via savepoint, dan pembatasan peran database. Reservasi bukan izin dispatch.
